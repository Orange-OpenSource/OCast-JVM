...
class ReferenceDeviceTest : HttpClientTest() {
    ...
    fun sendMessageWithMissingReceivedStatusFails() {
        ...
        val errorCaptor = argumentCaptor<OCastError>()
        verify(onError, times(1)).run(errorCaptor.capture())
        assertEquals(OCastError.Status.DEVICE_LAYER_MISSING_STATUS_ERROR.code, errorCaptor.firstValue.code)
    }
    ...
    fun sendMessageWithMissingReceivedReplyDataFails() {
        ...
        val errorCaptor = argumentCaptor<OCastError>()
        verify(onError, times(1)).run(errorCaptor.capture())
        assertEquals(OCastError.Status.DEVICE_LAYER_MISSING_REPLY_DATA_ERROR.code, errorCaptor.firstValue.code)
    }
